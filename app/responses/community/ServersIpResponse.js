...
module.exports = class ServersIPResponse extends ServerResponse {
  ...
  async run(client, message) {
    message.build(
      message.flyReply(`Vamos por partes... A Rede FlyHigh apresenta uma váriedade de servidores. Para descobrir o ip de algum servidor, você pode ir no <#> e ir no tópico do canal, igual é mostrado nessa imagem. E digitar o comando do servidor desejado!`, "826431055161983026"),
      ...
      `{ "option": { "allowedMentions": { "roles": [""], "users": ["${message.author.id}"] } } }`
    )
  }
}
